// Copied from https://github.com/PrismarineJS/prismarine-provider-anvil/blob/master/src/region.js
  ...
  async readAll () {
    ...
    for (let _x = 0; _x < 32; _x++) {
      for (let _z = 0; _z < 32; _z++) {
        if (this.hasChunk(_x, _z)) {
          keys.push([_x, _z])
          chunks.push(this.read(_x, _z))
        }
      }
    }
    return Promise.all(chunks).then(r => r.map((v, i) => [keys[i], v]))
  }
